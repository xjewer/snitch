...
func NewParser(r LogReader, s statsd.Statsd, cfg config.Source, l simplelog.Logger) (Parser, error) {
...
func (h *handler) HandleLine(l *Line) error {
	...
	for _, m := range h.metrics {
		...
		if m.count {
			h.l.Printf("EventKey: %s, value: 1", key)
		...
		if m.timing {
			...
			h.l.Printf("TimingKey: %s, value: %d", key, int64(1000*f))
